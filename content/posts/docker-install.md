...
## Step 4: ▶️ Start and Enable Docker
...
**⚠️ Important:** Log out and back in (or run `newgrp docker` in the current shell) for the group change to take effect.
...
sudo dnf config-manager --add-repo https://download.docker.com/linux/rhel/docker-ce.repo  # add repo 📦
sudo dnf install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin  # install 🚚

# Start it:
sudo systemctl enable --now docker  # ▶️
